...
new dhtmlDragAndDropObject;this.htmlNode=new dhtmlXTreeItemObject(this.rootId,"",0,this);this.htmlNode.htmlNode.childNodes[0].childNodes[0].style.display="none";this.htmlNode.htmlNode.childNodes[0].childNodes[0].childNodes[0].className="hiddenRow";this.allTree=this._createSelf();this.allTree.appendChild(this.htmlNode.htmlNode);dhtmlx.$customScroll&&dhtmlx.CustomScroll.enable(this);if(_isFF)this.allTree.childNodes[0].width="100%",this.allTree.childNodes[0].style.overflow="hidden";var e=this;this.allTree.onselectstart=
new Function("return false;");if(_isMacOS)this.allTree.oncontextmenu=function(a){return e._doContClick(a||window.event,!0)};this.allTree.onmousedown=function(a){return e._doContClick(a||window.event)};this.XMLLoader=new dtmlXMLLoaderObject(this._parseXMLTree,this,!0,this.no_cashe);_isIE&&this.preventIECashing(!0);window.addEventListener&&window.addEventListener("unload",function(){try{e.destructor()}catch(a){}},!1);window.attachEvent&&window.attachEvent("onunload",function(){try{e.destructor()}catch(a){}});
...
if(e.radio)h._r_logic=!0;if(e.nocheckbox){var j=h.span.parentNode.previousSibling.previousSibling;j.style.display="none";h.nocheckbox=!0}e.disabled&&(e.checked!=null&&this._setCheck(h,e.checked),this.disableCheckbox(h,1));h._acc=e.child||0;this.parserExtension&&this.parserExtension._parseExtension.call(this,a,e,b?b.id:0);this.setItemColor(h,e.aCol,e.sCol);e.locked=="1"&&this.lockItem(h.id,!0,!0);(e.imwidth||e.imheight)&&this.setIconSize(e.imwidth,e.imheight,h);(e.closeable=="0"||e.closeable=="1")&&
this.setItemCloseable(h,e.closeable);var i="";e.topoffset&&this.setItemTopOffset(h,e.topoffset);if(!this.slowParse||typeof this.waitUpdateXML=="object")a.sub_exists("item")&&(i=this._parse(a,e.id,1));if(i!="")this.nodeAskingCall=i;a.each("userdata",function(b){this.setUserData(a.get("id"),b.get("name"),b.content())},this)}}};
...
dhtmlXTreeObject.prototype._recreateBranch=function(a,b,c){var d,f="";if(c){for(d=0;d<b.childsCount;d++)if(b.childNodes[d]==c)break;d!=0?c=b.childNodes[d-1]:(f="TOP",c="")}var e=this._onradh;this._onradh=null;var g=this._attachChildNode(b,a.id,a.label,0,a.images[0],a.images[1],a.images[2],f,0,c);g._userdatalist=a._userdatalist;g.userData=a.userData.clone();if(a._attrs){g._attrs={};for(var h in a._attrs)g._attrs[h]=a._attrs[h]}g.XMLload=a.XMLload;if(e)this._onradh=e,this._onradh(g.id);for(d=0;d<a.childsCount;d++)this._recreateBranch(a.childNodes[d],
g,0,1);return g};
...
tree = new dhtmlXTreeObject("sample_treebox", "100%", "100%", 0);
tree.setSkin('dhx_skyblue');
tree.setImagePath("/images/dhtmlx/");
//tree.setXMLAutoLoading("php/json.php");
tree.setDataMode("json");
//load first level of tree;
tree.loadJSON("projects/index.json.jbuilder");
  
  //project_tree=  new dhtmlXTreeObject("sample_treebox","100%","100%",0);             	
  //project_tree.setImagePath(image_path);
  //project_tree.enableTreeImages("false");
  //project_tree.setXMLAutoLoading("/oligotype/xml/sample_xml_bpc.php");
             
  //project_tree.loadXML("/oligotype/xml/sample_xml_bpc.php?id=0");
              
  //project_tree.enableCheckBoxes("true");
  //project_tree.attachEvent("onCheck",sampleOnCheck);
  //project_tree.enableAutoTooltips("true");
  //project_tree.enableThreeStateCheckboxes("true");
  //project_tree.enableSmartXMLParsing("true");  //false to disable
  //project_tree.enableDistributedParsing("true");
