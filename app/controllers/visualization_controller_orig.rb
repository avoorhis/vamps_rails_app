class VisualizationController < ApplicationController
  ...
  def parse_view
    ...
    @dat_counts  = get_counts_per_dataset_id()
...
def get_uniq_seq_info_ids1(seq_ids_n_cnt_per_d)
  uniq_seq_info_ids_per_d = Hash.new{|hash, key| hash[key] = {}}
  seq_ids_n_cnt_per_d.each do |dataset_id, seq_i_hash|
    all_seq_ui = SequenceUniqInfo.where(sequence_id: seq_i_hash.keys())
    puts "all_seq_ui = " + all_seq_ui.inspect    
    puts "seq_i_hash = " + seq_i_hash.inspect    
    # seq_i_hash.each do ||
    #   uniq_seq_info_ids_per_d[dataset_id] << 
  end
  puts "uniq_seq_info_ids_per_d = " + uniq_seq_info_ids_per_d.inspect
  return uniq_seq_info_ids_per_d
end

# SELECT sum(seq_count) FROM sequence_pdr_infos
# JOIN datasets ON (datasets.id = dataset_id)
# WHERE dataset = 
# "SS_WWTP_1_25_11_2step"
# 2580224
# EXPLAIN
# SELECT sum(seq_count) FROM sequence_pdr_infos
# JOIN datasets ON (datasets.id = dataset_id)
# WHERE datasets.id IN (2,3,4)
def add_count_to_uniq_seq_info_ids_per_d(uniq_seq_info_ids_per_d, seq_ids_n_cnt_per_d)
  seq_ids_n_cnt_per_d.each do |dataset_id, seq_i_hash|
    seq_i_hash.each do |seq_id, seq_count|
      puts "seq_id = " + seq_id.inspect
      puts "seq_count = " + seq_count.inspect
      
      uniq_seq_info_ids_per_d.each do |dataset_id, usi|
        usi.each do |us|
          us.each do |u|
            if u.id == seq_id
              puts "u.taxonomy_id = " + u.taxonomy_id.inspect          
            end
          end
        end
      end
...
#     end
#   end  
#   puts "taxonomy_ids_per_d = " + taxonomy_ids_per_d.inspect
#   
